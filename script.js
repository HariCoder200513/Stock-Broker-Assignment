...
// Status constants (kept in sync with backend constants.py)
...
function populateSectorFilter() {
  ...
  const sectors = [
    ...new Set(state.stocks.map((stock) => stock.sector).filter(Boolean)),
  ].sort();
...
function renderTable() {
  ...
  const totalPages = Math.ceil(
    state.filteredStocks.length / state.itemsPerPage,
  );
  ...
  const rows = pageItems.map((stock) => {
    let statusBadge = "";
    if (stock.status === STATUS_SUCCESS) {
      statusBadge =
        stock.retries > 0
          ? `<span class="badge warning">Retried (${stock.retries})</span>`
          : '<span class="badge success">OK</span>';
    } else if (stock.status === STATUS_SKIPPED) {
      statusBadge = `<span class="badge warning" title="${stock.message || ""}">Skipped</span>`;
    } else {
...
function updateSummary(data) {
  ...
  document.getElementById("retriesCount").textContent =
    data.total_retries ?? "-";
...
function setupControls() {
  document
    ...
    .addEventListener("click", loadStocks);
  document
